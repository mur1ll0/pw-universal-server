...
habilidade, não da classe: no 1.5.5 são **cinco** (2909, 2910, 2913, 2914, 2917), todas da
**classe 11** — nenhuma classe antiga tem (B78).
...
| montaria (`SUMMON_PET` C2S 100 / `RECALL_PET` 101) | `testado` (B78) | invocar um mascote de montaria **é montar** (`PlayerSummonPet` → `pet_man::ActivePet`, `gs/player.cpp:14474-14491`, `gs/petman.cpp:319-392`). A velocidade é `speed_a + speed_b × (nível − 1)` do `PET_ESSENCE` (`pet_dataman::CalcMountParam`, `gs/petdataman.h:186-194`) e **sobrepõe** a de corrida; o cliente recebe `PLAYER_MOUNTING` (227, `{id, mount_id, u16 color}`) e a ficha nova, e desmontar manda o mesmo comando com zero nos dois (`mount_filter.cpp:24-45`, `player.cpp:14279-14319`). Recusa com `ERR_PET_IS_NOT_EXIST` (72), `ERR_PET_IS_ALEARY_ACTIVE` (71) ou `ERR_PET_CAN_NOT_MOUNT` (81). `falta`: mascote de **combate** (invocar a criatura), água, invisibilidade, transformação, a trava de ataque enquanto montado e a queda por lealdade |
