...
| avisos de missão (`TASK_VAR_DATA` 106) | `task_notify_base` = `reason u8, task u16`, `pack(1)`. `NEW`: + `cur_time u32, cap_task u32, sub_tags` (`sub_task u16, sz u8, tags[sz]`); `COMPLETE`: + `cur_time u32, sub_tags` com o **estado** no lugar do `sub_task`; `MONSTER_KILLED`: + `monster_id u32, num u16, dps i32, dph i32` = **17** bytes — no **1.2.6, 9 bytes, sem `dps`/`dph`** (captura da missão 1178: `04 9a04 e70c0000 0a00`; `WorldProtocol::task_notify_monster_killed`, B101; com 17 o cliente 1.2.6 descartava e o contador não andava). No 1.2.6 `NEW` (14 B), `COMPLETE` (10 B) e `ERROR_CODE` (7 B) batem com a captura; `ERROR_CODE`: + `u32`; `GIVE_UP`/`FINISHED`: só a base. O cliente confere o tamanho exato de cada um | `task/TaskTempl.h:1737-1875`, `TaskTempl.inl:2206-2286` |
...
| `PLAYER_CHGSHAPE` (163) | `{ int idPlayer; u8 shape }` = 2 + 5, ao dono **e** a quem está em volta; `shape` = `id \| (tipo << 6)`, **0 volta à forma normal**. Sai do `avisar_efeitos` quando a forma dos filtros difere de `PlayerEntity::forma_enviada` (Forma Sombria e raposa: 65 ao entrar, 0 ao sair). **1.2.6:** o byte é só o `_shape` (raposa = 1) — o `filter_Foxform::OnAttach` do `gs` 1.2.6 chama `ChangeShape(1)` (VA 0x830b1a0) e o `ChangeShape` guarda o valor inteiro como forma (VA 0x811d78a); `WorldProtocol::byte_de_forma` faz `& 0x3f` (B120). Quem chega depois vê a forma pelo `shape_form` do `info_player_1` no 1.5.5; o `info_player_1` do 1.2.6 não leva forma (falta medir) | `EC_GPDataType.h:2876-2880`, `gs/player.cpp:4892-4899`, `EC_Player.cpp:1901` (B95) |
